...
| `translation.json` | No | Aeneas manifest (emitted with the `emit-json` arg). When present, used as the authoritative loop/primary classification overlay; falls back to name heuristics when absent |
...
| `--translation <PATH>` | | Path to Aeneas's `translation.json` (`emit-json` arg). Optional authoritative loop/primary classification overlay. |
...
`functions.json` (which maps Rust qualified names to Lean names). Strategies
are used in priority order (the `charon-def-id` integer join first, then three
name/location strategies). Once an atom is matched by an earlier strategy, it is
excluded from later ones.

**Loop/primary classification.** Aeneas turns one Rust function with a loop into
several Lean defs — a top-level def plus `_loop` / `_loop.body` helpers, all
sharing the same `rust_name`. Each Rust atom maps 1-to-1 to the top-level def,
so the helpers must be deferred during matching. When Aeneas's `translation.json`
is available (via `--translation` or auto-detected under `PROJECT`), it is the
authoritative source: an entry is a loop helper iff its manifest record carries a
`loop` field (joined to `functions.json` by exact `lean_name`). Without the
manifest, classification falls back to name-suffix heuristics (`_loop`, `_body`,
`.body`). The manifest makes the outcome deterministic and immune to a real
function that merely happens to end in `_body`/`_loop`.

### 0. `charon-def-id` (confidence: `exact`)

Joins Rust and Lean by charon `FunDeclId` integer equality: the `charon-def-id`
extension field on a Rust atom equals the `def_id` in Aeneas's `translation.json`,
so equal ids bind the atom to that family's primary (non-loop) Lean def — precise,
